...
public class StarForgeCapabilityHandler implements IStarForgeCapability {
	...
	public void addEffect(IStarForgeEffect effect) {
		if (effect != null)
			setEffect(effect.getTier(), effect);
	}
	...
	public void setEffect(IStarForgeEffect.Tier tier, IStarForgeEffect effect) {
		switch (tier) {
			case ONE:
				if (effect != null && tier != effect.getTier())
					break;
				tier1 = effect;
				break;
			case TWO:
	...
	public void onEntityHit(Entity entityUser, Entity entityHit) {
		if (tier1 != null)
			tier1.onEntityHit(entityUser, entityHit);
		if (tier2 != null)
			tier2.onEntityHit(entityUser, entityHit);
		if (tier3 != null)
			tier3.onEntityHit(entityUser, entityHit);
	}

	@Override
	public void onBlockBreak(Entity entityUser, World world, IBlockState state, BlockPos pos, EnumFacing face) {
		if (tier1 != null)
			tier1.onBlockBreak(entityUser, world, state, pos, face);
		if (tier2 != null)
			tier2.onBlockBreak(entityUser, world, state, pos, face);
		if (tier3 != null)
			tier3.onBlockBreak(entityUser, world, state, pos, face);
	}
	...
	public boolean onRightClick(Entity entityUser) {
		boolean flag = false;
		if (tier1 != null)
			if (tier1.onRightClick(entityUser))
				flag = true;
		if (tier2 != null)
			if (tier2.onRightClick(entityUser))
				flag = true;
		if (tier3 != null)
			if (tier3.onRightClick(entityUser))
				flag = true;
		return flag;
	}
	...
	public boolean onRightClickBlock(Entity entityUser, World world, IBlockState state, BlockPos pos, EnumFacing face) {
		boolean flag = false;
		if (tier1 != null)
			if (tier1.onRightClickBlock(entityUser, world, state, pos, face))
				flag = true;
		if (tier2 != null)
			if (tier2.onRightClickBlock(entityUser, world, state, pos, face))
				flag = true;
		if (tier3 != null)
			if (tier3.onRightClickBlock(entityUser, world, state, pos, face))
				flag = true;
		return flag;
	}
