	...
	public void placeInPortal(Entity entityIn, float rotationYaw) {
		...
		if (entityIn.dimension == ConfigHandler.dimensionIdDalQuor) {
			...
			while (!worldServerInstance.isAirBlock(pos)) {
				pos = pos.up();
				if (worldServerInstance.getBlockState(pos).getBlock() == ModBlocks.dreamBed) {
					create = false;
					break;
				}
			}
			...
			if (create) {
				...
				IBlockState iblockstate2 = ModBlocks.dreamBed.getDefaultState().withProperty(BlockBed.OCCUPIED, Boolean.valueOf(false)).withProperty(BlockBed.FACING, EnumFacing.NORTH).withProperty(BlockBed.PART, BlockBed.EnumPartType.FOOT);
