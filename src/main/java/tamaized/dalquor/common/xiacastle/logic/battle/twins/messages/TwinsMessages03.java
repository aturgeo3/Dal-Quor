	...
	public static boolean run(World worldObj, BlockPos pos) {
		if (tick % childPhaseModulate == 0) {
			for (EntityPlayer p : worldObj.getEntitiesWithinAABB(EntityPlayer.class, new AxisAlignedBB(pos.add(-50, -50, -50), pos.add(50, 50, 50)))) {
				switch (childPhase) {
					case 0:
						p.sendMessage(new TextComponentTranslation("dalquor.twins.speech.riddle.3.1", p.getGameProfile().getName()));
						break;
					case 1:
						p.sendMessage(new TextComponentTranslation("dalquor.twins.speech.riddle.3.2"));
						break;
					case 2:
						...
						s.signText[0] = new TextComponentTranslation("dalquor.twins.riddle.3.1");
						s.signText[1] = new TextComponentTranslation("dalquor.twins.riddle.3.2");
						s.signText[2] = new TextComponentTranslation("dalquor.twins.riddle.3.3");
						s.signText[3] = new TextComponentTranslation("dalquor.twins.riddle.3.4");
						return true;
					default:
						break;
				}
