...
public class RenderSpellImplosion<T extends EntitySpellImplosion> extends Render<T> {
	...
	public void doRender(T entity, double x, double y, double z, float entityYaw, float ticks) {
		...
		for (int index = 0; index < 10; index++) {
			...
			float speed = 0.08F;
			Minecraft.getMinecraft().effectRenderer.addEffect(new ParticleFluff(world, entity.getPositionVector().add(0, 0.65F, 0).add(vec), new Vec3d(-vec.x * speed, -vec.y * speed, -vec.z * speed), 7, 0, rand.nextFloat() * 0.90F + 0.10F, 0x7700FFFF));
		}
	}
