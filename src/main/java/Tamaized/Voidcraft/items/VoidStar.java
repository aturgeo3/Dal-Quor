...
public class VoidStar extends TamItem {
	...
	public boolean onEntityItemUpdate(EntityItem entityItem) {
		if (entityItem.onGround && entityItem.dimension != voidCraft.config.getDimensionIDvoid() && entityItem.dimension != voidCraft.config.getDimensionIDxia() && entityItem.worldObj.getBlockState(new BlockPos(MathHelper.floor_double(entityItem.posX), MathHelper.floor_double(entityItem.posY - 1), MathHelper.floor_double(entityItem.posZ))).getBlock() == voidCraft.blocks.blockFakeBedrock // Make sure we're even ontop of a fakeBedrockBlock before we begin a loop
		) {
			for (int x = -1; x < 2; x++) {
				for (int z = -1; z < 2; z++) {
					for (int y = -1; y < 1; y++) {
