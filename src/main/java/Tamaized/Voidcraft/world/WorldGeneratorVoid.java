...
public class WorldGeneratorVoid implements IWorldGenerator {
	...
	private void generateEnd(World world, Random random, int BlockX, int BlockZ) {
		for(int i=0; i<6; i++) {
			...
			new WorldGenMinable(((OreVoidcrystal)voidCraft.blocks.oreVoidcrystal).getStateVoidFalse(), 5, BlockHelper.forBlock(Blocks.end_stone)).generate(world, random, new BlockPos(Xcoord, Ycoord, Zcoord));
		}
	}
}